<x-layout>
    <h1 class="text-3xl mb-5">Profile <span class="text-blue-500">{{ucfirst($user->name)}}</span></h1>
    <div class="lg:grid lg:grid-cols-2 gap-10">

        <form action="{{route('user-profile-information.update')}}" method="post" class="lg:col-span-1 bg-white p-5 rounded-md  shadow-xl" x-data="{ isLoading : false}" @submit="isLoading= true">
            @csrf
            @method('put')
        ...
        <form action="{{route('user-password.update')}}" method="post" class="lg:col-span-1 bg-white p-5 rounded-md  shadow-xl" x-data="{ isLoading : false}" @submit="isLoading= true">
            @csrf
            @method('put')
