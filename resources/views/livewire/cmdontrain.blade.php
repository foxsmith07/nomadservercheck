<div>
    ...
    <form wire:submit.prevent="check" method="POST" class="bg-white rounded-md shadow-xl p-5 flex flex-col gap-8">
        ...
        <div class="flex flex-col">
            ...
            <input type="text" wire:model="cmd" class="input-custom @error('cmd') outline! outline-red-500!  @enderror"
                value="{{ old('cmd') }}">
        </div>
        ...
        <div class="bg-white rounded-md shadow-xl p-7 w-full md:w-2/3 h-auto max-h-screen overflow-y-scroll">
            ...
            <div wire:loading.remove wire:target='check' class="mt-10">
                @if ($output or $output != null)
                    <div class="overflow-x-scroll">
                        ...
                        <pre class="{{ $this->nok == true ? 'text-red-500' : '' }}">{{ $output }}</pre>
            ...
            <div class="modal-action">
                <form method="dialog">
                    ...
                    <button class="btn" wire:loading.remove wire:target="check" wire:click="redirigi">
                        {{-- <a href="{{route('cmd.index')}}"> --}}
                        Close
                        {{-- </a> --}}
                    </button>
                </form>
